...
from src.video.google_studio import GoogleStudioAPI
...
def generate_video():
    try:
        ...
        title = request.form.get('title', 'Motivational Short')
        description = request.form.get('description', '')
        upload_to_youtube = request.form.get('upload_to_youtube', 'false') == 'true'
        
        # Generate or use custom text
        if custom_text:
            text = custom_text
        else:
            text = gemini_client.generate_motivational_quote(theme)
        ...
        if upload_to_youtube:
            try:
                youtube_response = google_studio.create_short(
